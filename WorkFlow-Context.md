...
| **TfLens's TF-013 to TF-045 and AppManager's TF-001 to TF-006 are fixed and deployed in all 19 repositories**; TfLens's reads 0 open, 15 waiting, 30 closed; AppManager closed all six on 2026-09-13. **AppManager's TF-007 (`tf-verify-tests.sh` and a `.slnx`-only solution) and TF-008 (`tf-build-list.py` labelling UI `trblazeui` without the library) are fixed on 2026-09-14 and deployed in all 19 repositories**; the replies are in AppManager's file and its copy here, and both re-checks already pass in AppManager. | AppManager (close) |
