...
| **TfLens's TF-013 to TF-050 and AppManager's TF-001 to TF-006 are fixed and deployed in all 19 repositories**; TfLens's reads 0 open, 16 waiting, 34 closed (TF-049, `tf-build.sh --print`, and TF-050, a spec's screenshot path read as a route, fixed 2026-09-14; TF-048, browser checks now take the build lock and run one at a time, FR-73, fixed 2026-09-14; TF-046, repeated icon findings, closed by TfLens the same day, and TF-047, a badge or border drawn one element away, an icon in another child and wrap on other text, both in `tf-mockup-parity` and both fixed 2026-09-14); AppManager closed all six on 2026-09-13. **AppManager's TF-007 and TF-008 are fixed on 2026-09-14 and closed by AppManager the same day; its TF-009 to TF-013 (test users in a `{App}-Usage-Guide.md`, the wrong web project booted, no way past an in-page sign-in for `tf-assets` and `tf-mockup-parity`, a password field read as the sign-in page, and the browser suite in one command) are fixed the same evening and deployed in all 19 repositories**; the reply is in AppManager's file and its copy here, every proof ran on AppManager's own booted admin site, and AppManager reads 0 open, 5 waiting, 8 closed. | AppManager (close) |
