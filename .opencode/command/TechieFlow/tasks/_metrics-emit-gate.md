...
1. **Agents never run `git` or `gh`** — not to emit, not to read, not to backfill. `.tfcore/hooks/block-git.sh` blocks it and is correct to. The only git-derived stream comes from the owner's own `pre-commit` hook, and the only script containing git commands (`.tfcore/telemetry/tf-metrics.sh`) is **owner-run**. If you think you need git for a metric, you need a different metric.
