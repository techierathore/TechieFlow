...
core_principles:
  ...
  - PERF ONLY AGAINST A DECLARED BUDGET - The §4c performance gate runs for a REQ if and only if its acceptance criteria carry a 'perf-budget:' line. Never invent a threshold, never grade a Debug build or a thin sample, and warn (PERF-MARGINAL) before you fail. A false perf failure teaches the owner to distrust every verdict you write, which costs more than the defect it would have caught.
