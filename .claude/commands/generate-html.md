...
Convert one or more markdown files to self-contained HTML using the shared shell. Use this when you need to render any documents that `*render-workflow-docs` does not cover (checklist docs, library-feedback docs, ad-hoc design notes, legacy/archived docs — note: suffixed `-v2`-style variants are banned by the collision policy; superseded docs live in `docs/OldDocs/`).

## Why this exists

`*render-workflow-docs` only renders the three canonical files (`{App}-BRD.md`, `{App}-Architecture.md`, `PROJECT-STATUS.md`). When the user has additional docs, this task accepts explicit paths.

## Invocation forms

```
/TechieFlow:tasks:generate-html @path/to/file.md
/TechieFlow:tasks:generate-html @path/to/file1.md @path/to/file2.md @path/to/file3.md
/TechieFlow:tasks:generate-html @docs/SomeFolder/
```
...
If no arguments are passed, HALT and tell the user: `Usage: /TechieFlow:tasks:generate-html @<path-to-md-or-dir> [@<more-paths>]`.
