...
| `misses.jsonl` | something an agent **missed** (`miss`), what repairing it cost (`miss-fix`), and a field completed later (`miss-amend`) | `verify-phase`, `build-phase`, `triage-issues`, `fix-issues`, `amend-docs`, `*log-miss` |
...
- **A field left empty can be completed later, never hand-edited.** `bash .tfcore/utils/tf-emit.sh --amend <miss_id> why_missed <value>` appends a third record kind, `miss-amend` (SCHEMA §5.5.7, added 2026-08-28 from TfLens feedback TF-005). It fills a `null` and **refuses to overwrite** anything else, so it adds to the history rather than revising it. Only closed-vocabulary judgements are amendable — attribution, tokens and cost never are. A miss written before a field existed leaves that field's denominator instead of counting as unassessed.
