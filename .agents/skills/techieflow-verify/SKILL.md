---
name: techieflow-verify
description: Independently verify requirements against runtime evidence. Use when a TechieFlow project needs its `verify-phase` workflow.
---

# techieflow-verify

1. Read `.tfcore/tasks/verify-phase.md` completely and follow it as the canonical executable workflow.
2. Read `.tfcore/core-config.yaml` and only the task dependencies required for this run.
3. Operate as the `verifier` role. Delegate only where the task explicitly calls for independent subagents, using the registered Codex roles and waiting for their results.
