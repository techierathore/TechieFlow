...
| FR-57 | checks the Architecture's Stack decisions table for a row per question 1 to 8 and 11, and refuses an app whose Solution structure names `<App>.App` or has no project named exactly the app. | script: the broken fixture fails on a missing Q3 row and on `MyDiary.App`. | MISS-TechieFlow-20260905-16; owner 2026-09-06 |
