...
> **Codex implementation addendum (2026-08-24):** the boundary is now
> three-way. Codex uses repository `AGENTS.md`, `.agents/skills/techieflow-*`,
> `.codex/agents/*.toml`, hooks, rules, and project config. Generation comes from
> canonical `.tfcore` content. Claude and OpenCode wiring remains unchanged.
...
> **Implementation status (2026-08-20, final):** EVERYTHING in this design is implemented and live-verified. Rollout §6 step 1: the guard-bridge plugin (§2.1, as `.opencode/plugin/techieflow.js`), the framework-owned `.opencode/opencode.jsonc` (§2.2; precedence VERIFIED; `{file:}` refs rewritten `../.tfcore/` — they resolve relative to the config file), the vocabulary edits (§2.3), `tf-harness.sh` (§2.4), the context layer (§2.5 — `app-agents-md-tmpl.md` + slim CLAUDE.md wrapper). Step 2's probes all PASSED (DECISIONS.md 2026-08-19 §7). The §5.3/§5.4 bindings are generated by **`.tfcore/utils/tf-routing-bind.sh`** (run automatically by `update-framework.sh`): Claude `/tf:<phase>` wrappers with `model:`/`effort:` + `.claude/agents/{tf-builder,tf-test-writer,tf-explorer,trblazeui,techierag}.md`; OpenCode `.opencode/opencode.json` (pure-JSON binding file — model-only agent entries deep-merge onto the deployed config with prompt/permission preserved, command entries carry template+model because each config file validates standalone; all verified against the 1.18.18 binary). Manifest-driven: `enabled: false` removes every generated artifact. What remains is the OWNER's step 3 experiment: flip `enabled: true` in one app's `routing.yaml`, run phases, read `tier/model/routed/tokens_*` off `runs.jsonl` (enrichment live — Telemetry-Hooks status note), and correct the §5.6 map with data.
