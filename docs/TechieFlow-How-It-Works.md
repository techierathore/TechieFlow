...
| **Template** | The blueprint for one document: a skeleton with placeholders and guidance for each section. The agent fills it to produce the BRD, the Architecture, and the other documents. Twenty-two exist under `.tfcore/templates/v4custom/`, fourteen of them opening with a schema block: the required sections in order, the size budget, and the rules every row must follow. `tf-doc-check.sh` reads that block and refuses a document that breaks it, so the shape is enforced rather than requested. | A document template plus a validator. |
...
| **Utility script** | A shell or Python script the agent runs instead of doing a step by hand, such as the HTML renderer or the telemetry writer. Fifty-four exist under `.tfcore/utils/`; the reset turned most of the framework's prose steps into one of them. | A build tool or CLI utility. |
...
| **Agent document** | A document whose only reader is an agent, so it stays markdown and is never rendered to HTML. Two are refused by name: the checklist and the miss list. | Internal working notes. |
| **Decision Request** | `docs/<App>-Decision-Request.md`. Where a choice only the owner can make goes: options, what each costs, a recommendation, and a block to paste back. Written in plain English, rendered for the owner, never read back by an agent. | A decision memo with a reply slip. |
...
| `*create-project-brief` | The brainstorm results | A one-page brief: product, users, must-do, out of scope | `app-brief-tmpl.md` | Used regularly. The brief is the input to day-1. **Kept 2026-09-08** and rebuilt: its own task file, a schema-backed template replacing the 221-line inherited one, and an OpenCode registration so both harnesses offer it. |
| `*create-competitor-analysis` | Owner input | Competitor analysis document | `competitor-analysis-tmpl.yaml` | Never used. **Removed 2026-09-08** (owner decision), with its template. |
| `*perform-market-research` | Owner input | Market research document | `market-research-tmpl.yaml` | Never used. **Removed 2026-09-08** (owner decision), with its template. |
...
flowchart TD
    ...
    H --- N["Always refused:<br/>git · writing Verified without a verify run ·<br/>malformed PROJECT-STATUS · test output outside tests/.artifacts ·<br/>hand edits of telemetry files · database writes outside build and fix ·<br/>a backgrounded build in YOLO · rendering an agent document to HTML ·<br/>ending the turn with the status gate incomplete<br/>(stale HTML, a failing PROJECT-STATUS or checklist, stale BRD table, no run record)"]
...
| D-14 | Harnesses | The framework still carries a Codex adapter. The owner supports two harnesses only. | **Closed 2026-09-07: the adapter is removed** from the framework, both delivery routes and all 23 projects (FR-42). TfLens's harness page will list Claude Code and OpenCode only. |
