...
**Phase 2 onward of a Large project** (`docs/<App>-P<n>-BRD.md`, template `app-phase-brd-tmpl.md`, 2026-09-11, TfLens TF-024). The phase-1 BRD above holds everything that belongs to the whole application; a later phase's BRD holds only its own screens and requirements, because two copies of one fact become two answers the day one is amended. Its sections, in the same order as above: Summary, Screens and flow, Requirements, Non-functional requirements (only when the phase adds its own), Development status, and **Where the rest lives** — a table that must link to the phase-1 BRD. Scope, Users and roles, Context diagram, Constraints and assumptions and Risks are not in it, and the checker calls them strangers if they appear. Same budget and checks as above. Until this template existed every phase BRD was graded against the whole-project one, which asked for the six sections it must not have: 20 findings on TfLens's two phase BRDs, none repairable without copying phase 1.
...
**Phase 2 onward of a Large project** (`docs/<App>-P<n>-UIDesign.md`, template `app-phase-uidesign-tmpl.md`, owner 2026-09-11). The same split as the phase BRD in §3.1. Phase 1 keeps what every phase shares: the UI library and theme in its header, the Design system, the one Click-through flow across every phase and the Branding guide. A later phase's UIDesign holds only its own `### Screen:` entries, written and checked exactly as above, and a **Where the rest lives** table that must link to the phase-1 UIDesign. Until this template existed each phase UIDesign was asked for the phase-1 header rows and sections: 10 findings on TfLens's two phase files, none repairable without copying phase 1.
