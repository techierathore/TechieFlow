...
## 0. Current implementation status (as of 2026-08-28)
...
| `cost_attribution` derived from the fix run's `reqs_touched` | **Shipped** | `tf-emit.sh` |
| Collapse check (one defect = one miss) | **Shipped** | `tf-emit.sh --open-miss`, SCHEMA §5.5.4 |
| Report: `analyse_misses()` / `print_misses()`, both exclusions enforced **in code** | **Shipped** | `.tfcore/telemetry/tf-metrics.sh` |
| Six front doors — `verify-phase` §6a · `build-phase` §4a + FIX-mode close · `triage-issues` §6a · `fix-issues` §6a · `amend-docs` §7a · **`*log-miss`** | **Shipped** | `.tfcore/tasks/*.md` (+ byte-identical Claude mirror) |
| `*log-miss {App} "<what was missed>" [--fixed]` command, help, deps, routing | **Shipped** | `.tfcore/tasks/log-miss.md`, `.tfcore/agents/flow-master.md` |
| Constraint 10 (attribution / cost / collapse) | **Shipped** | `.tfcore/tasks/_metrics-emit-gate.md` |
| Report section + template §5 | **Shipped** | `metrics-report.md`, `templates/v4custom/metrics-report-template.md` |
| Harness parity — Claude mirror, `opencode.jsonc`, Codex regen (9 agents / 19 skills) | **Shipped** | mirrors verified byte-identical |
| Human docs — `WORKFLOW.html` §17 + TOC/quick-refs/FAQ, `README.md` §7.13 + §17/§17a, Telemetry Guide §3.5 | **Shipped** | — |
| `.gitattributes` | **No change needed**, exactly as §5.1 predicted (the `docs/metrics/*.jsonl` glob already covers it) | — |

**Verified end to end** on two throwaway repos: id issue → collapse check (opens once, silent on re-verify, empty after a `Verified` close) → `shared:3` vs `sole` derived correctly from `reqs_touched` → a deliberately fabricated `origin_model` overwritten by the looked-up one → an unlinked origin downgraded to `inferred` with the model nulled → the report refusing every figure under n=3 and printing both exclusions with the counts that bound them.

**Live in this repo:** all five streams collecting; `project_type: framework`; 30 commit records reconstructed back to 2026-06-25 by `--backfill-commits`.
...
### 0.3 As-built deltas from the design below

The design shipped substantially as written. Seven differences are worth knowing, and where they conflict with §4–§8, **the code and SCHEMA §5.5 win**:

1. **`why_missed` is on the record** (SCHEMA §5.5.6) — ported from the Playbook and reflected in §4.3's field table. Seven values: the Playbook's six plus TechieFlow's own **`instruction-ignored`**. Optional, but **mandatory in practice on an escape** (`triage-issues` treats it as not-optional), and the report warns via `escapes_missing_why` when one arrives without it. Its distribution is reported against **records that carry the field**, never against all misses — a `null` is "not assessed", not a zero in some bucket.
...
- Whether **`instruction-ignored`** should be offered back to the Playbook. Deliberately not assumed — see the sibling doc §7.
- `_metrics-emit-gate.md` constraint 1 still calls `tf-metrics.sh` flatly "owner-run" although `--report` / `--rollup` contain no git call. Narrowing a safety rule is the owner's decision, so it was flagged, not edited.
...
| `why_missed` | *(added 2026-08-28, ported from the Playbook — see the sibling doc §7)* Which **practice** failed: `missing-checklist-item` · `insufficient-verify-method` · `code-audit-limitation` · `ambiguous-acceptance` · `dependency-not-declared` · **`instruction-ignored`** · `other`. Optional; `null` = not assessed. TechieFlow adds `instruction-ignored` — an agent skipping a written step is this framework's dominant failure mode and the Playbook's list has no slot for it. |
...
| `origin_confidence` | string | `linked` (`origin_run_id` resolved to a real run record) · `inferred` (attributed by reasoning, no run record found) · `unknown`. **As built: derived by `tf-emit.sh`, never written by an agent** — see §0.3 item 2. |
...
| `fix_cmd` | string | `fix-issues` \| `build-phase` \| `triage-issues` \| `amend-docs` \| `log-miss` (**as built** — the `--fixed` mode, §0.3 item 3) |
