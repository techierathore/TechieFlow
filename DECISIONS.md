...
## 2026-08-21 — YOLO / goal mode: run unattended to completion; delete prompt moves into the hook; whole-checklist build passes

**Trigger.** The owner ran an end-to-end goal on a VM and it took three days — not computing, but waiting: Claude Code and OpenCode prompted for every `rm`, blocked every git read, paused at phase boundaries and elicitation points, stopped on the 5-hour usage limit, and `*build-phase` kept ending with "run build-phase again for the remaining REQs". Owner rule: *`*yolo` / `/goal` means I have given you all permissions and all access; delete anything, read git, never commit; stop only when the goal is complete; wait out usage limits yourself.*

**Decisions.**

1. **One YOLO definition, four triggers, one flag.** `*yolo`, the word YOLO anywhere in a command, an active Claude Code `/goal`, or a `tf-goal.sh` run all mean the same thing — `.tfcore/tasks/_yolo-mode.md` (new shared rule, mirrored). State is `.tfcore/.session/yolo.json` (`tf-yolo.sh on|off|status|done`), never committed. The hook also treats the payload's `permission_mode` of `bypassPermissions`/`auto` as YOLO, so a bypass-mode session is YOLO even if the agent forgot the flag.
2. **The delete/sudo prompt moved OUT of `settings.json` `ask` and INTO the hook.** Verified against the docs (permissions.md "Extend permissions with hooks"; permission-modes.md): a settings `ask` rule prompts **even in `bypassPermissions` mode and even when a PreToolUse hook returns `allow`** — deny → ask → hook → allow. So as long as `Bash(rm *)` sat in `ask`, *no* mode could suppress it; that is precisely the prompt that stalled the VM run. A hook-issued `permissionDecision: "ask"` is equivalent in normal mode and can be withheld in YOLO. `settings.json` `ask` is now `[]`.
3. **Git: deny WRITES in settings (every mode), decide READS in the hook.** `Bash(git *)` was a deny — and deny rules hold in bypass mode, so YOLO could never read git. The deny list is now the enumerated write subcommands (`git commit|push|add|rm|mv|reset|checkout|switch|restore|merge|rebase|cherry-pick|revert|clean|am|apply|init|clone|pull|fetch|…`, `gh pr|issue|repo|release|workflow|run|secret|variable|label|auth|gist … create|merge|close|edit|delete|…`). `block-git.sh` was rewritten from a regex into a shell-aware classifier (python3 + `shlex`; splits on `; && || | & ( )`, `$(…)`, backticks; recurses into `bash -c`/`eval`; skips `sudo|env|xargs|timeout|winrun` wrappers; honours `git -C/-c/--no-pager` globals): **write → exit 2 always; read → exit 2 outside YOLO, allow in YOLO**; `rm|rmdir|unlink|shred|find -delete|sudo` → hook `ask` outside YOLO, silent in YOLO. Falls back to the original regex when python3 is missing or the command does not tokenize. 59-case battery in the session scratchpad passed (compound forms, wrappers, `branch -d` vs `branch -a`, `gh api -X POST` vs bare `gh api`, false positives like `.gitignore`/`github.com`/grepping for the word).
4. **OpenCode parity.** `opencode.jsonc`'s seven bash maps (root + six agents — the P-0 rule from 2026-08-20 still applies: every agent repeats the full map) now deny the same write list and keep `rm */rmdir */sudo *: ask`; the plugin gained `permission.ask` (auto-`allow` when the flag is on) and exports `TF_YOLO=1` to tool shells. Headless: `opencode run --auto`.
5. **Usage-limit waits live OUTSIDE the session.** Nothing inside a turn can sleep five hours, so `.tfcore/utils/tf-goal.sh` (new) supervises: launches `claude -p --permission-mode bypassPermissions --output-format stream-json` (or `opencode run --auto`), classifies each exit from the output tail — **LIMIT** (parses `usage limit reached|<epoch>`, `resets 7pm (Asia/Kolkata)`, `resets in 2h 14m`, `retry-after`, ISO, weekly `resets Tue 3pm`; unparseable → **probe** with a one-turn `claude -p "Reply OK"` every `--probe-min` 15 until it answers cleanly, max `--probe-max-hours` 8) → sleep until reset **+ 15 min** (`--buffer-min`) and `--resume <session_id>`; **CRASH** → 2→30 min backoff; **IDLE** (agent stopped without finishing) → re-prompt. Stops only on the agent's sentinel `tf-yolo.sh done complete|blocked` (exit 0 / 3), or `--max-cycles` (4). State + log under `.tfcore/.session/goal.*`; `--resume <app>` survives a reboot. The docs do not specify the headless limit message format, hence the pattern set rather than one regex.
6. **A build pass is the whole working list — in every mode.** `build-phase.md §2b` (new): the pass ends when every §2a working-list REQ is ≥ `Implemented` or a logged `Blocked`/owner-gated blocker, the verifier has been chained, and (YOLO) its FAIL rows looped (`§6c`, ≤5 FIX cycles, each emitting `mode:"fix"`). "Run build-phase again for the remaining REQs" is banned as an ending; context pressure ⇒ more sub-agent clusters. `_status-update-gate.md` item 5 carries the matching rule so the status gate cannot launder the early stop into a "next command".
7. **What YOLO does NOT relax:** `guard-verify.sh` (no self-attested `Verified`), `guard-status.sh`, the smoke policy, the status gate, the no-git-write rule. YOLO removes waiting, not evidence.

**Propagation.** `update-framework.sh` refreshes `settings.json` to the new canonical block by default (`--keep-permissions` opts out), force-syncs `hooks/` + `utils/` (the rewritten hook, `tf-yolo.sh`, `tf-goal.sh`), tasks (incl. `_yolo-mode.md`), agents, and the OpenCode plugin + framework `opencode.jsonc`; both scaffolds carry the same block. Apps scaffolded before today keep prompting on `rm` until they are updated — that is the first thing to check when a YOLO run still asks.

**Not done, on purpose.** No runtime model switching on limit (Capability-Matrix, 2026-08-19 still stands); no attempt to auto-detect a limit from inside the session; `/goal`'s own completion judge is left to Claude Code — the framework's sentinel is the authoritative "done" for the supervisor because it is written only after the status gate. No git was run.

---

## 2026-08-21 — Advisory escalation policy in model routing (ported from the AI-First Playbook)

`.tfcore/routing.yaml` gains an `escalation:` block, shipped as `fix-issues: after_attempts: 2, tier: frontier`. Ported from the Playbook's `playbook/model-tiers.yml` `escalation:` block and `docs/Model-Routing-Guide.md` §4; the Playbook's own decision record (`docs/Decisions.md`, "runtime auto-escalator" rejected) is adopted verbatim in spirit.

**The decision: escalation is advisory, applied at launch — never a runtime model switch.**

- **What it means.** When the same REQs have already been through `<phase>` `after_attempts` times without reaching `Verified`, the *next* run should be launched on `tier`. The policy is read by whoever launches the phase (the owner, or a wrapper script) from the attempt history **before** the run starts: the `attempt` field on `runs.jsonl` records (new, see below), with the per-REQ verdict history in `gates.jsonl` and the `PROJECT-STATUS.md` Verification Log as the human-readable view. `bash .tfcore/utils/tf-emit.sh --next-run-attempt <phase> <REQ-ID>...` prints the number the next record would get.
- **Why not runtime.** Neither harness can change a running turn's model (Capability-Matrix, 2026-08-19). The one structural possibility — OpenCode's `chat.message` model mutation — is undocumented API with no Claude Code equivalent; building on it would make escalation a one-harness feature and the Claude Code daily path a second-class citizen. The Playbook reached the same verdict: "dumb, visible, portable" beats clever and half-supported.
- **Why not enforced at launch either.** Enforcement would need a launcher that sits between the owner and the harness on every invocation. TechieFlow's owner control is one script that edits a file; adding a mandatory launch gate for one phase is a worse trade than an advisory line in `status`. The telemetry (`attempt`, `tier`, `model`, `routed` per run) is what makes the threshold tunable — a second-attempt failure rate that stays high says "lower it"; third attempts that succeed on the base tier say "raise it".

**What changed (additive; routing stays `enabled: false`; the Claude Code daily path is untouched):**

1. `.tfcore/routing.yaml` — `escalation:` section (4-space children, same flat-parse contract). `tf-harness.sh` / `tf-routing-bind.sh` parsers are section-gated and ignore it (verified). `update-framework.sh` already preserves a deployed `routing.yaml`, so existing apps pick the block up only via `set-escalation` or a hand edit — by design, same as every other routing change.
2. `.tfcore/utils/tf-routing.sh` — `status` prints the policy next to each phase's base tier; new `set-escalation <phase> <attempts> <tier>` (updates an existing row, adds a row, or creates the section; validates the integer and tier; does **not** re-bind because escalation generates no binding).
3. `.tfcore/utils/tf-emit.sh` — `attempt` stamped on `runs` records that carry a non-empty `reqs_touched`: `1 +` prior non-backfilled `run` records with the same `cmd` whose `reqs_touched` intersects (batch-aware; backfilled and REQ-less records are left alone). New read helper `--next-run-attempt <cmd> <REQ>...`. Same fail-open contract — prints `1` / leaves the field absent on any error. `SCHEMA.md` §2.5 documents it, distinct from the gate-level per-REQ `attempt` of §3.1.
4. Docs — Routing Guide §6.4 + troubleshooting row; README §17b and WORKFLOW.html §17b one-paragraph summaries.

**Not done, on purpose:** no task template (`fix-issues.md`) was changed — the phase does not read the policy, because a task reading it could only *report* the advice, and `status` already does. No git was run.
...
6. **S-1 vocabulary neutralisation (same day, third pass):** the three mandatory fan-out sites now name both harnesses' mechanisms — `build-phase.md` §3 `[flow-master]` clusters, `verify-phase.md` §4 test-writer spawn, `fix-issues.md` §4 data/logic route each read "**`tf-builder`/`tf-test-writer` if the harness registers it, otherwise the harness's general subagent** (Claude Code: `Agent` tool, `subagent_type=…|general-purpose`; OpenCode: `task` tool, `subagent_type=…|general`)". `SCHEMA.md` `runs.subagents` enum gained `general` + `tf-builder`/`tf-test-writer`/`tf-explorer`. Mirrored byte-identical to `.claude/commands/TechieFlow/tasks/`. The tf-* names are forward references to the routing subagents — harmless until those agents are registered (the "if the harness registers it" guard).
7. **Routing declaration shipped, OFF (same pass):** `.tfcore/routing.yaml` (Adapter-Design §5.2 verbatim: three tiers with per-harness ids — Claude `opus|sonnet|haiku`, OpenCode ids from this machine's `opencode models` as starting values; the full §5.6 phase map; subagent tiers; effort map; `enabled: false`) and `.tfcore/utils/tf-harness.sh` (§2.4: `detect|root|enabled|tier|model|effort|invoke|session`, flat-YAML awk parsing, graceful `inherit`/`unknown` answers, TF_HARNESS-first detection matching tf-emit). `update-framework.sh` deploys `routing.yaml` once and then PRESERVES it (per-project — the rollout flips one app at a time and tier→model maps are per-machine); `tf-harness.sh` rides the `utils/` force-sync. **Flipping `enabled: true` alone changes nothing by design** — the §5.3/§5.4 harness bindings (Claude `/tf:<phase>` wrapper commands + `tf-*` agents; OpenCode command/agent `model` fields) are the enable-time step, still to be built when the owner starts the routing experiment.
8. **Documentation aligned (same pass):** README §0 gained "OpenCode in WSL — the primary path" (install, PATH-before-the-npm-shim, what the scripts auto-deploy, the `/mnt/c`-vs-ext4 rule) and the Docker section is marked FALLBACK ONLY; WORKFLOW.html §0 same treatment; `build-invocation-ladder.md` §E and `opencode-operating-contract.md` runtime classification marked Docker-as-fallback / WSL-as-expected; `TechieFlow-OpenCode-Gaps.md` got a 2026-08-20 supersession pointer; `Adapter-Design.md`/`Telemetry-Hooks.md` carry implementation-status headers; Coupling-Points S-1 marked FIXED.

9. **Everything remaining, closed (same day, fourth pass — the design is fully implemented).**
   - **T-3 per-run telemetry:** new `session-pointer.sh` hook (SessionStart + UserPromptSubmit → `.tfcore/.session/claude-code.json`; wired in `.claude/settings.json` and all three scripts' canonical heredocs); `tf-emit.sh` `enrich_run()` adds `tier/tier_model/model/models/routed/tokens_in/out/cache_*/cost_usd/tokens_scope` to `runs`/`gates` records carrying `started`+`ended` — Claude window over the pointer transcript (scope `main`, `cost_usd` always `null` — never estimated), OpenCode window over `opencode.db` message rows for the pointer session + descendants (`parent_id` BFS; scope `tree`, real cost; pinned to `message.data` JSON, not columns), fallback `tokens_scope:"none"`. All three paths live-tested against the real stores. `SCHEMA.md` §2.5 documents the fields + the never-pool-dollars rule.
   - **Routing bindings (Adapter-Design §5.3/§5.4):** new `.tfcore/utils/tf-routing-bind.sh` generates, from `routing.yaml` when `enabled: true`: Claude `/tf:<phase>` wrapper commands (`model:`/`effort:` frontmatter, body = load owner persona + execute task) + `.claude/agents/{tf-builder,tf-test-writer,tf-explorer,trblazeui,techierag}.md`; OpenCode `.opencode/opencode.json` — a pure-JSON binding file that loads alongside the framework `.jsonc`. Two merge facts verified against the deployed 1.18.18 binary: **each config file validates standalone** (command entries must carry `template`, so generated ones are complete) and **model-only agent entries deep-merge** (prompt/permission preserved, model overridden). Manifest-driven cleanup: `enabled: false` removes exactly what was generated (23 artifacts round-tripped in test). `update-framework.sh` runs it per app.
   - **C-1 context layer:** new `app-agents-md-tmpl.md` — day-1 now writes the CONTENT into a **committed `AGENTS.md`** (required reading, hard rules incl. both harnesses' git-ban enforcement, basics, REQ prefixes, verification, command table); `app-claude-md-tmpl.md` became a thin gitignored wrapper (`@AGENTS.md` + Claude-only permissions/tool-preference). day1-brownfield/greenfield §7/§7.2 updated. Existing apps migrate at their next day-1/amend — nothing breaks meanwhile (their current files still load).
   - **C-2:** the "Evidence discipline" doctrine added to `_smoke-test-policy.md` (both harnesses load it); the operating contract marks its copies secondary. **P-1:** `rm */rmdir */sudo *: ask` added to `opencode.jsonc` root + all six agent maps (after the wildcard allow — last-match-wins). **I-1:** the five `@`-sigil sites now carry the "plain path under OpenCode" note. **E-2:** ladder paths de-hardcoded to `$HOME/...`. **A-1:** resolved by design (§4 list). Cosmetics: N-4/N-5/N-7/N-9 fixed, N-1 kept-dual-form by decision, N-2 via C-1, N-6 resolved (plugin type dep), N-3/N-8 acknowledged stale, untouched.
   - Propagated to all 18 repos (verified per repo: hook + scripts + templates + config + no stray `.opencode/opencode.json` while disabled).

10. **The remaining UNVERIFIED items, probed (same day, fifth pass).** (a) **Claude `SubagentStop` — VERIFIED** via a nested headless `claude -p` session with the hook wired: `transcript_path` is the parent's, but the payload carries a dedicated **`agent_transcript_path`** → `<transcript-dir>/<session-id>/subagents/agent-<id>.jsonl`, same JSONL/`message.usage` format. Because that path is deterministic, `tf-emit.sh` `_window_claude` now globs the subagents dir beside the pointer transcript — **no hook needed** — and Claude runs report `tokens_scope:"tree"` when subagents exist (`"main"` otherwise). Live-tested against a real subagent session (parent+child tokens summed). (b) **OpenCode routed-command model persistence — VERIFIED at state level:** the command's model is written to the session row (probe: `kimi-k2.6` stored while the project default was `kimi-k3`), so model-less `run -c`/`-s`/SDK continuations inherit the command's tier, as the source read predicted. (c) **One-turn parallel `task` — VERIFIED CONCURRENT:** two `task` calls in one assistant message produced child sessions created 35ms apart with ~6s overlapped busy windows — build-phase's mandatory parallel fan-out works under OpenCode with no experimental flag. **The one sliver left — closed same day by the owner's TUI test:** the TUI does **not** re-impose its own model selection; a plain follow-up after a routed command continues on the command's model (matches the session-row persistence in (b); the earlier "TUI re-sends its selection" guess in Adapter-Design §5.4 was wrong and is corrected). Owner-facing rule added to README/WORKFLOW §17b: after a routed phase, re-pick your model (or start a new session) if you keep chatting. With this, **no claim in the harness-adapter design remains unverified.** Also added same day: `.tfcore/utils/tf-routing.sh` (`status|on|off|set-tier|set-model|bind`) — the one-command owner control that edits `routing.yaml` and re-binds automatically; README §17b and WORKFLOW.html §17b rewritten as task-oriented guides around it. Side observation from the probes: `opencode run` intermittently hung at init (before session creation) three times and recovered on retry — consistent with the update/catalog check; `autoupdate: false` in the global config (deployment guide §3) is the mitigation. Claude Code path untouched except additive hooks/enrichment and identical-behavior wording.
