...
**Not done yet (deliberately):** D-1 propagation (`update-framework.sh` deploying the plugin + config to scaffolded apps), the framework-owned `.opencode/opencode.jsonc` split, vocabulary edits (S-1), routing.yaml, Claude-side session pointer. Claude Code path untouched except the additive tf-emit detection branch.
