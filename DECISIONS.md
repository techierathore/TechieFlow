...
**Not done yet (deliberately):** the routing enable-time bindings (§5.3/§5.4 generation), the Claude-side session pointer hook, per-run token enrichment in `tf-emit.sh` (`tier`/`model`/`tokens_*` on `runs`/`gates`). Claude Code path untouched except the additive tf-emit detection branch and the identical-behavior vocabulary wording.
