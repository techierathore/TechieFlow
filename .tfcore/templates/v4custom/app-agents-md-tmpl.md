# {AppName} — project session memory (all harnesses)
...
1. **Git is manual — agents NEVER run `git` or `gh`.** Not to commit, and not to read (`status`/`log`/`diff`/`grep`/`blame`). Both harnesses enforce this mechanically: Claude Code via `.claude/settings.json` DENY + the `block-git.sh` PreToolUse hook (compound forms included); OpenCode via `opencode.jsonc` `permission.bash` denies (tree-sitter-checked per command node) + the `.opencode/plugin/techieflow.js` guard bridge running the same script. A blocked git call is the policy working, not an obstacle to route around. Evidence for status updates / "what changed" = the checklist Requirements Status table + the working-tree files (+ mtimes) + a fresh `dotnet build` (`.tfcore/tasks/_status-update-gate.md`). The OWNER commits, in a separate terminal.
...
If `/trblazeui` or `/techierag` is missing: run `dotnet build` (NuGet deploy target), then `update-framework.sh` (copies the persona into `.claude/commands/`), then restart the harness.
