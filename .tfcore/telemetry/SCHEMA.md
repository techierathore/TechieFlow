...
**A run with no `reqs_touched` is still costed — the divisor is misses, not REQs (fixed 2026-08-28).** `reqs_touched` is empty in every `framework` and `docs` repo, because those projects have no REQs to touch; until this date the derivation fell through to `none` there, so **a measured window was discarded as unattributable and the cost of fixing a framework's own misses was structurally zero.** That contradicted this table's own definition of `none` — the run had resolved and `tokens_scope` was `main`. Same blind spot as `project_type:"framework"` (§0.63), one stream over. Where `reqs_touched` is empty the divisor is now **the number of distinct misses that run closed**, counted from `miss_id`s sharing the `fix_run_id`.

That count is not complete at emit time — an append-only stream writes one `miss-fix` at a time, so a run closing four misses stamps `shared:1` … `shared:4` and only the last is right. So **`tf-metrics.sh` recomputes the divisor per `fix_run_id` at report time** and does not trust the stored value (§8: derived metrics are computed, never stored). This is what makes the fix retroactive: records already carrying `none` are re-costed from data already on the stream, with no record edited and no backfill. The report prints the recovered count so a jump in the cost figures reads as a corrected derivation rather than as work suddenly costing more. `sole` is still `sole`, and a record with no window is still `none` — the recomputation only ever fills in a share that was countable all along.
