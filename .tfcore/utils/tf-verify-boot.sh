...
#   bash .tfcore/utils/tf-verify-boot.sh status
#
# Heads:
...
# The state goes to tests/.artifacts/verify/boot.json; the app log to tests/.artifacts/verify/app.log.
# `stop` kills what `start` started and nothing else. Never asks anyone to start anything.
# Exit 0 booted / stopped · 2 NONE (reason printed; kind=build-error, host or no-driver) · 3 usage.
set -u
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DIR="tests/.artifacts/verify"; STATE="$DIR/boot.json"; LOG="$DIR/app.log"
mkdir -p "$DIR"
...
json.dump({"head": head, "mode": mode, "url": url, "pids": [int(p) for p in pids.split() if p.isdigit()],
           "rung": rung, "project": project, "reason": reason, "reason_kind": kind, "stopped": False,
           "platform": sys.argv[9] if len(sys.argv) > 9 else "", "log": "tests/.artifacts/verify/app.log",
           "started": datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")},
          open("tests/.artifacts/verify/boot.json", "w"), indent=1)
PY
}
...
cmd="${1:-}"; shift || true
case "$cmd" in
  status) if [[ -f "$STATE" ]]; then cat "$STATE"; else echo "NONE no boot state"; fi; exit 0 ;;
  stop)
    [[ -f "$STATE" ]] || { echo "STOPPED nothing was started"; exit 0; }
    python3 - <<'PY'
import json, os, signal, subprocess, time
s = json.load(open("tests/.artifacts/verify/boot.json"))
...
s["stopped"] = True
json.dump(s, open("tests/.artifacts/verify/boot.json", "w"), indent=1)
print(f"STOPPED head={s.get('head')} mode={s.get('mode')} pids={s.get('pids')}")
PY
    exit 0 ;;
  start) ;;
  *) sed -n '2,26p' "${BASH_SOURCE[0]}" | sed 's/^# \{0,1\}//'; exit 3 ;;
esac
...
if [[ "$HEAD" == "static" ]]; then
  ...
  if poll_http "http://localhost:$PORT/" 20 "$PID"; then
    ...
    echo "BOOTED head=static mode=base url=http://localhost:$PORT pid=$PID log=$LOG"; exit 0
  fi
...
if [[ "$HEAD" == "web" ]]; then
  ...
  for r in "${rungs[@]}"; do
    ...
    if poll_http "$URL/" 120 "$PID"; then
      ...
      [[ -n "$WP_PORT" ]] && python3 -c "import json;p='$STATE';s=json.load(open(p));s['win_port']=$WP_PORT;json.dump(s,open(p,'w'),indent=1)"
      echo "BOOTED head=web mode=base url=$URL rung=$label project=$PROJECT pid=$PID log=$LOG"; exit 0
    fi
...
CDP_PORT=9222; RELAY_PORT="${PORT:-9223}"
...
python3 -c "import json;p='$STATE';s=json.load(open(p));s['win_image']='$ASM.exe';json.dump(s,open(p,'w'),indent=1)"
